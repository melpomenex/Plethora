...
fn write_app_default_permission(manifest_dir: &Path, commands: &[String]) {
...
"#,
    ...
    let path = permissions_dir.join("default.toml");
    if std::fs::read_to_string(&path).ok().as_deref() != Some(body.as_str()) {
        std::fs::write(&path, body).expect("write permissions/default.toml");
    }
...
fn main() {
    ...
    write_app_default_permission(manifest_dir, &app_commands);
