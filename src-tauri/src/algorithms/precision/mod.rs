...
//! interval = min_growth_guard(interval, used)
//! defaults : w = [6, 14, 45, 25, 10]
//! ```
//!
//! The post-lapse/normal choice is made per review from THIS review's grade
//! and the pre-review counters; post-lapse therefore shortens the lapsed
...
pub struct PrecisionState {
    ...
    // Written by the binary's interval_dispatch and read by
    // the finalizer to select the post-lapse path
    // over the normal dispersal path. The
    ...
    /// (Classic/Classic15/Classic19/Precision/FSRS slot order). Used to score each
...
/// Result of a Precision review.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrecisionReviewResult {
    pub state: PrecisionState,
    pub interval_days: f64,
    pub retrievability: f64,
    /// Deterministically finalized intervals for Classic/Classic15/Classic19/Precision/FSRS.
...
pub struct PrecisionCollectionState {
    ...
    /// competitor runs real FSRS with these weights instead of the default formulas.
    pub fsrs_params: Option<Vec<f32>>,
    /// Per-user Precision kernel parameters (35 doubles) from the optimizer.
...
/// Map the app's 4-button rating to the Precision 0-5 grade scale.
///
/// In this scale, grades 0-2 are FAIL variants and 3-5 are PASS
/// variants — every model in this module branches on `grade >= 3`. The
/// canonical 4-button ↔ 6-grade mapping is:
...
pub fn review(
    ...
    // Update M1 history point with the finalized interval
...
/// Preview intervals for every native grade 0-5 (deterministic).
///
...
pub fn preview_grade_results(
    ...
    pure_m4: bool,
    post_lapse_x: f64,
) -> [PrecisionReviewResult; 6] {
    // Clone collection state so we don't mutate it during preview
    let mut coll = PrecisionCollectionState {
...
/// Initialize a new Precision item for the given grade.
pub fn init_item(grade: i32) -> PrecisionState {
...
mod tests {
    ...
    fn arena_preview_is_deterministic_complete_and_non_mutating() {
        ...
        assert!(
            (commit_collection.arena.decayed_baseline_loss - expected_arena.decayed_baseline_loss).abs()
                < 1e-14
        );
        assert_eq!(
