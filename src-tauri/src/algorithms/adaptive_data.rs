//! Plethora Adaptive SInc Matrix Data Loader
//!
...
/// Active SInc matrix dimension (0..20 = 20 active buckets: 0..=19)
pub const SINC_ACTIVE_DIM: usize = 20;
...
pub fn sinc_matrix_index(d: usize, s: usize, r: usize) -> Option<usize> {
    // Validate indices are within active range (0..20, i.e., 0..=19)
    if d >= SINC_ACTIVE_DIM || s >= SINC_ACTIVE_DIM || r >= SINC_ACTIVE_DIM {
        return None;
    }
...
mod tests {
    ...
    fn test_spot_check_row_boundaries() {
        ...
        // Index 418: End of first D-slice active region (D=0, S=19, R=19)
        let last_d0_active = 0 * 441 + 19 * 21 + 19;
        assert!(
            matrix[last_d0_active] > 0.0,
            "matrix[{}] should be positive, got {}",
            last_d0_active,
            matrix[last_d0_active]
        );
