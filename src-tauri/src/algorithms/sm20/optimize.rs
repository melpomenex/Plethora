...
//!
//! - **M4 / Algorithm SM-20**: fits the 35-double kernel parameter block to
...
//!   they beat the defaults on unseen items. This replicates what SuperMemo
//!   sells as SM-20 weight optimization "via API", locally.
//! - **M5 / FSRS**: converts the review log into `fsrs-rs` training items so
//!   the crate's own battle-tested optimizer (`compute_parameters`) can fit
