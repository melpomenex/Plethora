...
lazy_static! {
    static ref API_KEY_REGEX: regex::Regex = regex::Regex::new(r#""INNERTUBE_API_KEY"\s*:\s*"([^"]+)""#).unwrap();
...
fn select_caption_track(tracks: &[CaptionTrack], requested_lang: Option<&str>) -> Option<CaptionTrack> {
    if tracks.is_empty() {
        return None;
    }
    ...
    let prefix_match = tracks.iter()
        .filter(|t| t.language_code.split('-').next().unwrap_or(&t.language_code) == prefix_lang)
        .min_by_key(|t| t.kind.as_deref() == Some("asr"));
...
async fn attempt_fetch(
    ...
    if status_code == 429 {
        return Err(("RateLimited".to_string(), "YouTube returned 429 Rate Limited".to_string()));
    }
    if !status_code.is_success() {
        return Err(("Network".to_string(), format!("YouTube API returned status code {}", status_code)));
    }
    
    let body = response.text().await.map_err(|e| ("Network".to_string(), e.to_string()))?;
    let player_json: serde_json::Value = serde_json::from_str(&body)
        .map_err(|e| ("Unknown".to_string(), format!("Failed to parse player JSON: {}", e)))?;
        
    let playability = player_json.get("playabilityStatus");
    let status = playability.and_then(|p| p.get("status").and_then(|s| s.as_str())).unwrap_or("OK");
    let reason = playability.and_then(|p| p.get("reason").and_then(|r| r.as_str())).unwrap_or("");
    ...
    if status != "OK" {
        if status == "LOGIN_REQUIRED" {
            ...
            return Err(("Unknown".to_string(), format!("Playability status: {}, reason: {}", status, reason)));
        }
    }
    
    let tracks_json = player_json.get("captions")
    ...
    let tracks: Vec<CaptionTrack> = match tracks_json {
        Some(t) => serde_json::from_value(t.clone())
            .map_err(|e| ("Unknown".to_string(), format!("Failed to parse caption tracks: {}", e)))?,
        None => return Err(("NoCaptions".to_string(), "No caption tracks in player response".to_string())),
    };
    
    if tracks.is_empty() {
        return Err(("NoCaptions".to_string(), "No caption tracks available".to_string()));
    }
    
    let selected_track = select_caption_track(&tracks, language)
        .ok_or_else(|| ("NoCaptions".to_string(), "Failed to select caption track".to_string()))?;
        
    let caption_url = selected_track.base_url;
    let gating_marker = caption_url.contains("signature=") || !caption_url.contains("&sig=");
    if gating_marker {
        return Err(("PoTokenRequired".to_string(), "Caption track URL is signature-gated".to_string()));
    }
    
    let format_url = format!("{}&fmt=json3", caption_url);
    
    let caption_res = client.get(&format_url)
        .header("User-Agent", ctx.user_agent)
        .timeout(Duration::from_secs(5))
        .send()
        .await
    ...
    if caption_status == 403 {
        return Err(("PoTokenRequired".to_string(), "Caption track download forbidden (403)".to_string()));
    }
    if !caption_status.is_success() {
        return Err(("Network".to_string(), format!("Caption track fetch returned status {}", caption_status)));
    }
    
    let caption_body = caption_res.text().await.map_err(|e| ("Network".to_string(), e.to_string()))?;
    if caption_body.trim().is_empty() {
        return Err(("PoTokenRequired".to_string(), "Caption track response is empty".to_string()));
    }
    
    let json3_data: Json3Response = serde_json::from_str(&caption_body)
        .map_err(|e| ("Unknown".to_string(), format!("Failed to parse json3 transcript: {}", e)))?;
        
    let mut segments: Vec<TranscriptSegment> = Vec::new();
    for event in json3_data.events.unwrap_or_default() {
        let text_raw = event.segs.unwrap_or_default().iter()
            .filter_map(|s| s.utf8.as_ref())
            .map(|s| s.as_str())
            .collect::<Vec<_>>()
            .join("");
        let text = strip_tags(&decode_html_entities(&text_raw)).trim().to_string();
        if text.is_empty() {
            continue;
        }
        
        // Collapse consecutive exact duplicate texts
        ...
        let start = event.t_start_ms.unwrap_or(0.0) / 1000.0;
        let duration = event.d_duration_ms.unwrap_or(0.0) / 1000.0;
...
pub async fn fetch_youtube_transcript_on_device_internal(
    ...
    for ctx in CLIENT_CONTEXTS {
        match attempt_fetch(&client, video_id, &api_key, ctx, language).await {
            Ok((segments, lang)) => {
                return OnDeviceTranscriptResult::Ok { segments, language: lang };
            }
            Err((kind, detail)) => {
...
mod tests {
    ...
    #[test]
    fn test_strip_tags() {
        assert_eq!(strip_tags("<font color=\"#ffffff\">hello</font> <b>world</b>"), "hello world");
    }
