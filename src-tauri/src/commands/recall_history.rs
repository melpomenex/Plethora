...
mod tests {
    ...
    async fn fetch_recent(
        pool: &sqlx::SqlitePool,
        document_id: Option<&str>,
    ) -> Vec<RecallPromptRecord> {
        let cutoff =
            (chrono::Utc::now() - chrono::Duration::days(RECALL_DEDUP_WINDOW_DAYS)).to_rfc3339();
        let rows = match document_id {
            Some(doc) => {
                sqlx::query(
                    "SELECT id, document_id, chunk_ids, fingerprint, question, asked_at, outcome \
    ...
    async fn test_round_trip_and_json_chunk_ids() {
        ...
        insert(
            ...
            Some("doc-1"),
            "fp-a",
            "Why can virtual memory exceed RAM?",
            &now_rfc3339(),
            "asked",
        )
        .await;
    ...
    async fn test_document_filter_and_window() {
        ...
        let old = (chrono::Utc::now() - chrono::Duration::days(RECALL_DEDUP_WINDOW_DAYS + 5))
            .to_rfc3339();
        insert(
            &pool,
            "r-old",
        ...
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM recall_prompt_history WHERE id = 'r-old'")
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(count, 1);
    ...
    async fn test_prune_deletes_only_old_rows() {
        ...
        .await;
        let ancient =
            (chrono::Utc::now() - chrono::Duration::days(RECALL_PRUNE_DAYS + 10)).to_rfc3339();
        insert(
            &pool,
            "r-ancient",
            Some("doc-1"),
        ...
        let deleted = prune_recall_history(&pool, RECALL_PRUNE_DAYS)
            .await
            .unwrap();
        assert_eq!(deleted, 1);

        let ids: Vec<String> =
            sqlx::query_scalar("SELECT id FROM recall_prompt_history ORDER BY id")
                .fetch_all(&pool)
                .await
                .unwrap();
    ...
    async fn test_outcome_update() {
        ...
        let updated =
            sqlx::query("UPDATE recall_prompt_history SET outcome = 'answered' WHERE id = 'r-out'")
                .execute(&pool)
                .await
                .unwrap();
        ...
        let outcome: String =
            sqlx::query_scalar("SELECT outcome FROM recall_prompt_history WHERE id = 'r-out'")
                .fetch_one(&pool)
                .await
                .unwrap();
