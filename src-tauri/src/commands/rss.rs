...
pub async fn create_rss_feed(
    ...
    .execute(repo.pool())
    .await
    .map_err(|e| crate::error::PlethoraError::Internal(format!("Failed to create RSS feed: {}", e)))?;
...
pub async fn get_rss_feeds(
    ...
    let rows = sqlx::query("SELECT * FROM rss_feeds WHERE collection_id = ? ORDER BY title")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to fetch RSS feeds: {}", e))
        })?;
    ...
    for row in rows {
        feeds.push(RssFeed {
            ...
            collection_id: row
                ...
                .unwrap_or_else(|_| crate::models::collection::DEFAULT_COLLECTION_ID.to_string()),
        });
    }
...
pub async fn get_rss_feed(id: String, repo: State<'_, Repository>) -> Result<Option<RssFeed>> {
    let row = sqlx::query("SELECT * FROM rss_feeds WHERE id = ?")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to fetch RSS feed: {}", e))
        })?;
...
pub async fn update_rss_feed(
    ...
    {
        return get_rss_feed(id, repo)
            .await?
            .ok_or_else(|| crate::error::PlethoraError::NotFound("Feed not found".to_string()));
    }
    ...
    query.execute(repo.pool()).await.map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to update RSS feed: {}", e))
    })?;

    get_rss_feed(id, repo)
        .await?
        .ok_or_else(|| crate::error::PlethoraError::NotFound("Feed not found".to_string()))
}
...
pub async fn delete_rss_feed(id: String, repo: State<'_, Repository>) -> Result<()> {
    sqlx::query("DELETE FROM rss_feeds WHERE id = ?")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to delete RSS feed: {}", e))
        })?;
...
async fn upsert_rss_article(pool: &sqlx::SqlitePool, input: RssArticleInput) -> Result<RssArticle> {
    ...
    if let Some(row) = existing_by_guid {
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to update RSS article: {}", e))
        })?;
        ...
        .execute(pool)
        .await
        .map_err(|e| crate::error::PlethoraError::Internal(format!("Failed to create RSS article: {}", e)))?;
    }
...
pub async fn get_rss_articles(
    ...
    let rows = if let Some(ref fid) = feed_id {
        sqlx::query(
            "SELECT * FROM rss_articles WHERE feed_id = ? ORDER BY published_date DESC LIMIT ?",
        )
        .bind(fid)
    ...
    .map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to fetch RSS articles: {}", e))
    })?;
...
#[tauri::command]
pub async fn mark_rss_article_read(
    id: String,
    is_read: bool,
    repo: State<'_, Repository>,
) -> Result<()> {
    sqlx::query("UPDATE rss_articles SET is_read = ? WHERE id = ?")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to mark article: {}", e))
        })?;
...
pub async fn mark_rss_feed_read(feed_id: String, repo: State<'_, Repository>) -> Result<()> {
    sqlx::query("UPDATE rss_articles SET is_read = 1 WHERE feed_id = ? AND is_read = 0")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to mark feed read: {}", e))
        })?;
...
pub async fn update_rss_feed_fetched(id: String, repo: State<'_, Repository>) -> Result<()> {
    ...
    sqlx::query("UPDATE rss_feeds SET last_fetched = ? WHERE id = ?")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to update feed: {}", e))
        })?;
...
#[tauri::command]
pub async fn get_rss_feed_unread_count(
    feed_id: String,
    repo: State<'_, Repository>,
) -> Result<i32> {
    let row =
        sqlx::query("SELECT COUNT(*) as count FROM rss_articles WHERE feed_id = ? AND is_read = 0")
            .bind(&feed_id)
            .fetch_one(repo.pool())
            .await
            .map_err(|e| {
...
pub async fn cleanup_old_rss_articles(days: i32, repo: State<'_, Repository>) -> Result<i32> {
    ...
    let result = sqlx::query("DELETE FROM rss_articles WHERE date_added < ? AND is_queued = 0")
        ...
        .await
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to cleanup articles: {}", e))
        })?;
...
pub async fn fetch_rss_feed_url(feed_url: String) -> Result<ParsedFeed> {
    ...
    crate::security::validate_url_not_private(&normalized_feed_url)
        .map_err(|e| crate::error::PlethoraError::Internal(format!("URL not allowed: {}", e)))?;

    let client = Client::builder()
        .user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
        .build()
        .map_err(|e| {
            crate::error::PlethoraError::Internal(format!("Failed to create HTTP client: {}", e))
        })?;

    let response = client.get(&normalized_feed_url).send().await.map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to fetch feed: {}", e))
    })?;
    ...
    let xml_bytes = response.bytes().await.map_err(|e| {
    ...
    let doc = Document::parse(&xml_content).map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to parse XML: {}", e))
    })?;
...
fn parse_rss_item_node(item: &roxmltree::Node) -> Option<ParsedFeedItem> {
    ...
    let author = item
        ...
        .or_else(|| {
            item.children()
                .find(|n| n.tag_name().name() == "dc:creator")
        })
        .and_then(|n| n.text())
        .map(|s| s.to_string());
...
pub async fn update_rss_feed_http(
    ...
    if title.is_none()
        ...
        && is_active.is_none()
    {
        return get_rss_feed_http(id, repo)
            .await?
            .ok_or_else(|| crate::error::PlethoraError::NotFound("Feed not found".to_string()));
    }
...
pub async fn fetch_article_full_content(
    ...
    let client = Client::builder()
        ...
        .build()
        .map_err(|e| crate::error::PlethoraError::Internal(format!("Failed to create HTTP client: {}", e)))?;
    ...
    .await
    .map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to store full content: {}", e))
    })?;
...
pub async fn update_feed_auto_fetch(
    ...
    .await
    .map_err(|e| {
        crate::error::PlethoraError::Internal(format!("Failed to update feed auto-fetch: {}", e))
    })?;
...
pub async fn get_article_full_content(
    ...
    let row =
        sqlx::query("SELECT full_content, full_content_fetched_at FROM rss_articles WHERE id = ?")
            .bind(&article_id)
            .fetch_optional(repo.pool())
            .await
            .map_err(|e| {
                crate::error::PlethoraError::Internal(format!(
                    "Failed to get full content: {}",
                    e
                ))
            })?;
