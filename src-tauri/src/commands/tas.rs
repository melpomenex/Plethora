...
pub async fn build_tas_queue(
    ...
    let config = match repo.get_setting("tas_config").await? {
        Some(json) => serde_json::from_str::<TASConfig>(&json).unwrap_or_default(),
    ...
    let tags_by_name: HashMap<String, Tag> =
    ...
    for tag in tags_by_name.values() {
        ...
        tag_stats.insert(
            tag.name.clone(),
            recompute_tag_stability_stats(&stabilities, tag),
        );
    }
...
/// Get all tags
#[tauri::command]
pub async fn get_tags(repo: State<'_, Repository>) -> Result<Vec<Tag>> {
    repo.get_all_tags().await
}

/// Create or update a tag
#[tauri::command]
pub async fn upsert_tag(repo: State<'_, Repository>, tag: TagInput) -> Result<Tag> {
    repo.upsert_tag(
        &tag.name,
...
#[tauri::command]
pub async fn delete_tag(repo: State<'_, Repository>, tag_id: String) -> Result<()> {
...
#[tauri::command]
pub async fn sync_tags(repo: State<'_, Repository>) -> Result<usize> {
...
#[tauri::command]
pub async fn compute_tag_centroids(repo: State<'_, Repository>) -> Result<usize> {
