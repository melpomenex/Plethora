...
// Upstream bug workaround (wry #1775 / tauri iOS+macOS dev): WebKit marshals
// every off-main `WKURLSchemeTask did*` call through
// `callOnMainRunLoopAndWait`, an unbounded synchronous wait on the app's main
// run loop. When the responder is invoked from an async runtime worker while
// the main thread is itself blocked (e.g. tauri's `safe_block_on` dev-server
// proxy fetch), both threads park on each other. With enough in-flight
// responses the whole worker pool wedges, taking the I/O driver with it.
// Fix: when off the main thread, dispatch the entire did* sequence onto the
// GCD main queue, which is serial FIFO so per-task ordering (didReceiveResponse
// before didReceiveData before didFinish) is preserved, and return
...
mod main_queue {
  ...
  pub fn dispatch_to_main(f: impl FnOnce() + 'static) {
    // RcBlock requires an `Fn` closure; Cell gives the one-shot FnOnce
    // semantics (a second invocation would be a no-op) without FnMut.
    ...
    unsafe {
      dispatch_async(
        std::ptr::addr_of!(MAIN_QUEUE) as *mut c_void,
        (&*block as *const Block<dyn Fn()>) as *mut c_void,
      );
    }
  }
}
        ...
        Ok(final_request) => {
          ...
          let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
            Box::new(move |sent_response| {
              ...
              // Own the id for the closure that may be dispatched to the main
              // queue; everything else is already owned by this responder.
              ...
              // See `mod main_queue` above: never let WebKit's
              // callOnMainRunLoopAndWait synchronously wait on a main run
              // loop that may itself be blocked waiting on this response.
              if main_queue::is_main() {
                respond();
              } else {
                main_queue::dispatch_to_main(respond);
              }
            });
