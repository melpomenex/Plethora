...
vi.mock('../database', () => ({
  ...
  getSyncState: vi.fn(),
  commitBrowserPrecisionReview: vi.fn(),
...
import { currentDayFromCe, freshPrecisionCollectionState } from '../precisionScheduler';
import parityFixture from '../../shared/arenaParityFixture.json';

const legacyPrecisionCollectionKey = `${"s"}m${"20"}_collection_state`;
const legacyPrecisionAlgo = `${"s"}m${"20"}`;
const legacyArenaModel = `${"s"}m${"19"}`;

describe('browser Precision collection parity', () => {
  ...
  it('uses persisted M2/M3/Arena state and commits the item and learner together', async () => {
    ...
    const storedCollection = freshPrecisionCollectionState();
    vi.mocked(db.getLearningItem).mockResolvedValue(item);
    vi.mocked(db.getSyncState).mockImplementation(async (key) =>
      key === 'precision_collection_state' ? storedCollection : undefined);
    vi.mocked(db.commitBrowserPrecisionReview).mockResolvedValue(undefined);
    ...
    expect(db.commitBrowserPrecisionReview).toHaveBeenCalledOnce();
    const [committedItem, committedCollection] = vi.mocked(db.commitBrowserPrecisionReview).mock.calls[0];
  ...
  it('reads legacy collection key when canonical key is absent', async () => {
    const item = browserItem();
    const storedCollection = freshPrecisionCollectionState();
    vi.mocked(db.getLearningItem).mockResolvedValue(item);
    vi.mocked(db.getSyncState).mockImplementation(async (key) =>
      key === legacyPrecisionCollectionKey ? storedCollection : undefined);
    vi.mocked(db.commitBrowserPrecisionReview).mockResolvedValue(undefined);
    ...
    await browserInvoke('submit_review', {
      ...
      rating: 3,
      grade: parityFixture.committed_grade,
      algorithm: legacyPrecisionAlgo,
    });

    expect(db.getSyncState).toHaveBeenCalledWith(legacyPrecisionCollectionKey);
    expect(db.commitBrowserPrecisionReview).toHaveBeenCalledOnce();
  });
  ...
  it('atomically commits and snapshots the full collection for an Arena choice', async () => {
    ...
    expect(provenance.schedule_model_id).toBe(legacyArenaModel);
    expect((committedCollection as typeof storedCollection).m2_optimizer.cell_cases.flat()
      .reduce((sum, value) => sum + value, 0)).toBe(1);
