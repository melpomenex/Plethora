...
export interface SM20CollectionState {
  schema_version: 1;
...
export function freshSm20CollectionState(): SM20CollectionState {
  return {
    schema_version: 1,
...
export function parseSm20CollectionState(value: unknown): SM20CollectionState {
  if (!value || typeof value !== 'object') return freshSm20CollectionState();
  const input = value as Partial<SM20CollectionState>;
...
function sanitizeArenaState(state: ArenaState): ArenaState {
  ...
  return {
    ...
    decayed_sm19_loss: validDiagnostics ? state.decayed_sm19_loss : 0,
...
export function observeArena(
  ...
  arena.decayed_sm19_loss = arena.decayed_sm19_loss * 0.995 + losses[2];
