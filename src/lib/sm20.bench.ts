/**
 * SM-20 scheduling hot path — repetition scheduling over a fixed grade
 * sequence. The scheduler ensemble runs per grade, per item: it is the
 * tightest loop in the app (see src/lib/sm20.ts).
 *
...
import { freshSm20CollectionState, parseSm20State, sm20Review, type SM20State } from "./sm20";

const BASE_STATE = parseSm20State();
...
// A fixed "today" in Chrono days-from-CE range (≥ 100_000): parseSm20State
...
function runReviewSequence(): number {
  let state: SM20State = BASE_STATE;
  let acc = 0;
