...
export type { SM20CollectionState } from './sm20Collection';
export { freshSm20CollectionState, currentDayFromCe } from './sm20Collection';
...
export interface SM20ExecutionOptions {
...
/** Review an SM-20 item using the native-equivalent 5-model ensemble. */
export function sm20Review(
  ...
  const collection = options.collection ?? freshSm20CollectionState();
...
export function sm20PreviewGradeResults(
  currentState: SM20State,
  elapsedDays: number,
  pureM4?: boolean,
  collection?: SM20CollectionState,
...
export function sm20PreviewIntervals(
  ...
  const grades = sm20PreviewGradeResults(currentState, elapsedDays, pureM4, collection, today);
  return {
