...
function initDifficultyScalar(w: Fsrs7Parameters, rating: number): number {
  const r = clamp(Math.round(rating), 1, 4);
  return w[4] - Math.exp(w[5] * (r - 1)) + 1.0;
}

function linearDampingScalar(deltaD: number, oldD: number): number {
  return ((10.0 - oldD) * deltaD) / 9.0;
}

function meanReversionScalar(init: number, current: number): number {
  return init * 0.01 + current * 0.99;
}

function nextDifficultyScalarForRetention(
  w: Fsrs7Parameters,
  d: number,
  rating: number,
  retention: number,
  ...
  return clamp(
    meanReversionScalar(initDifficultyScalar(w, 4), newD),
    D_MIN,
    D_MAX,
  );
}

function stabilityForSetScalar(
  w: Fsrs7Parameters,
  lastS: number,
  lastD: number,
...
function fastComponentRecallScalar(w: Fsrs7Parameters, t: number, sFast: number): number {
...
export function forgettingCurveScalarForState(
  w: Fsrs7Parameters,
  t: number,
  state: Fsrs7MemoryState,
): number {
  const elapsed = Math.max(0.0, t);
...
function forgettingCurveAndDerivativeScalarForState(
  w: Fsrs7Parameters,
  t: number,
  state: Fsrs7MemoryState,
): [number, number] {
...
function nextIntervalBisectionScalarForState(
  w: Fsrs7Parameters,
  state: Fsrs7MemoryState,
...
export function nextInterval(
  ...
  const clampedState: Fsrs7MemoryState = {
...
export function nextStateScalar(
  w: Fsrs7Parameters,
  state: Fsrs7MemoryState,
  deltaT: number,
  rating: number,
  ...
  const newSSlow = stabilityForSetScalar(
    w,
    state.stability,
    state.difficulty,
    retrievability,
  ...
  let newSFast = stabilityForSetScalar(
    w,
...
function isInitialState(state: Fsrs7MemoryState): boolean {
  return state.stability === 0 && state.difficulty === 0 && state.stability_fast === 0;
}

function initStabilityScalar(w: Fsrs7Parameters, rating: number): number {
  const r = clamp(Math.round(rating), 1, 4);
  return w[r - 1];
}

export function stepMemoryState(
  w: Fsrs7Parameters,
  current: Fsrs7MemoryState,
  deltaT: number,
  rating: number,
  nth: number,
): Fsrs7MemoryState {
  ...
  let next = nextStateScalar(
    w,
    { stability: lastS, difficulty: lastD, stability_fast: lastSFast },
    deltaT,
    rating,
  );
...
function clampInterval(interval: number, maximumInterval?: number): number {
...
export function nextStatesWithElapsedDays(
  w: Fsrs7Parameters,
  currentMemoryState: Fsrs7MemoryState | null | undefined,
  daysElapsed: number,
  options: Fsrs7ScheduleOptions = {},
): Fsrs7NextStates {
  ...
  const buildForRating = (rating: number): Fsrs7ItemState => {
