...
- **Binary IPC for file reads** — PDF byte ranges and whole-document reads now cross the Rust↔WebView bridge as raw bytes (`ArrayBuffer`) instead of JSON number arrays / base64 strings. Each 512 KB PDF page-fetch previously serialized to ~1.8 MB of JSON text and re-parsed in the webview — the dominant cost of PDF paging on Android; whole-file reads paid a 33% base64 inflation plus an `atob` pass on desktop. Text consumers switched from `atob` to `TextDecoder`, which also fixes mojibake for non-ASCII characters in imported HTML. The Android 16 MiB inline-read guard is preserved.
- **Queue mutations apply locally instead of full reloads** — postpone/suspend/delete now patch the affected item in place instead of re-transferring the entire queue after every action (8 full-reload call sites removed; queue views reconcile with a fresh listing when refocused). This is the dominant queue win, and it removes flicker on every action. Also fixes a latent bug where smart-postponing a *document* was routed to the learning-item lookup and failed with NotFound. (A payload-slimming option for queue listings exists behind an opt-in flag but is intentionally off by default — the shared queue state also feeds semantic search, the knowledge graph, and schedule titles, which need full card content.)
- **Smaller, better-split startup bundle** — The entry chunk drops from 1,444 KB to 1,308 KB: the full 65-theme registry was being merged into the startup bundle by a static+dynamic dual import in `ThemeContext` (the intended lazy load was silently defeated); fallback themes now live in their own module. Settings went from one 961 KB chunk to a 43 KB shell with 17 per-section lazy chunks (the 511 KB handbook only loads if opened). The duplicated PDF.js worker build is gone (~1.06 MB saved; one worker asset serves the worker-port, workerSrc-fallback, and PWA paths). A `check-bundle-budget` script wired into `build:check` fails the build on size regressions.
- **Mobile queue is virtualized** — The mobile queue list previously mounted a DOM row for every item; above 20 items it now windows rows through the same virtual list as the desktop queue (preserving swipe gestures, pull-to-refresh, and the scroll-restore contract), keeping large queues smooth on Android.
- **Blocking work off the async runtime** — Document file reads use async I/O and CPU-bound PDF text extraction runs on the blocking thread pool in all four extraction paths (three ran synchronously on tokio workers), so a large import or extraction no longer stalls unrelated IPC commands. Whole-store zustand subscriptions in the queue/review/settings hot paths were converted to selectors, trimming unnecessary re-renders.
...
- **Knowledge Universe** — Replaced the 2D Knowledge Sphere with a stunning, high-performance 3D deterministic galaxy layout powered by Three.js. It features cluster-based category starbursts, smooth camera transitions, dynamic search warp animations, keyboard navigation, and a robust WebGL failure fallback to the classic 2D sphere.

  ![Knowledge Universe](https://raw.githubusercontent.com/melpomenex/Incrementum/main/openspec/changes/knowledge-universe/knowledge-universe.png)

- **Browser Extension Improvements** — Enhanced options page, popup logic, theme synchronization, and highlight exporting options in the browser extension helper.

### Fixed & Improved

- **Yjs Sync Performance** — Optimized Yjs sync startup replay, database transactions, clock caching, and replication telemetry to significantly reduce startup overhead and synchronization latency.
...
- **Corrected the reverse-engineered SM-20 V4 interpretation** — Raw `sm20.exe` assembly and UI strings establish that V4 is an “Approximation of recall,” fitted from empirical pass/total counts. Incrementum no longer treats that probability surface as an interval or stability-increase multiplier and no longer feeds it through the unrelated V2 Bayesian interval matrices. Existing matrix data is retained for rollback but is inactive.
- **Local SM-20 recall optimizer** — SM-20 reviews now accumulate compact retrievability/difficulty pass statistics and can fit the executable's four-coefficient V4 recall surface entirely on the user's device with bounded deterministic coordinate search. The profile is explicitly diagnostic-only until the separate recall-to-interval path has reference parity; no cloud API is needed and fitting cannot silently change schedules.
...
- **Deprecated the legacy REST sync server routes** (`/sync`, `/files`) with `Deprecation`/`Sunset` headers ahead of their planned removal.
...
### Added
- side-by-side thumbnail rendering on all screens in RSS Scroll Mode.

### Fixed & Improved
- Optimized the RSS Scroll Mode view on mobile devices to reclaim significant vertical screen real estate.
- Reduced title font sizes (`text-base` to `text-lg` instead of `text-2xl` on mobile) and adjusted line-height to fit headlines in fewer lines.
- Compacted padding on mobile action buttons (`Show cover image`, `View Full Content`), categories list, feed indicator, and footer buttons.
- Re-adjusted header padding to prevent overlay collision with absolute top navigation controls.
