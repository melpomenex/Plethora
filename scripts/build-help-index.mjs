...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  try {
    const existing = JSON.parse(fs.readFileSync(OUTPUT_SRC_HELP_JSON, "utf8"));
    existingHash = existing.corpusHash;
  } catch {
    // first run or missing output
  }

  if (existingHash === index.corpusHash) {
    console.log(`Help index unchanged (${index.corpusHash.slice(0, 19)}...); skipping rewrite.`);
    process.exit(0);
  }
