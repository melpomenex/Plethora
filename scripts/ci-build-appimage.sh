...
build_rc=0
APPIMAGE_EXTRACT_AND_RUN=1 npm run tauri -- build --bundles appimage || build_rc=$?
...
if [[ ! -f "$EXPECTED_APPIMAGE" ]]; then
  ...
  if [[ ! -f "$ROOT_DIR/Incrementum-x86_64.AppImage" ]]; then
    echo "Fallback appimagetool packaging did not produce Incrementum-x86_64.AppImage"
    exit 1
  fi
