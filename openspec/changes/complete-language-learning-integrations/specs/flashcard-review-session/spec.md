...
The Review tab's session queue SHALL be populated exclusively from due flashcards / learning items (the `getDueItems` stream). The queue SHALL NOT include reading items (documents) under any condition. Reading-item review SHALL remain available in the Queue / Optimal Queue tab, which is a separate surface. Language practice attempts SHALL remain outside the review queue unless the learner explicitly creates a learning item through the shared SRS flow.
...
#### Scenario: Practice result does not enter review automatically
- **WHEN** a learner completes a language practice attempt without choosing Memorize or an explicit learning-item action
- **THEN** the attempt SHALL remain practice history/evidence and SHALL NOT appear in the Review session queue

### Requirement: Reading items are reviewable only in the Queue

Reading items (documents) SHALL be reviewed through the Queue / Optimal Queue tab (its `reading` mode / "Start Optimal Session"), which remains unchanged. The Review tab SHALL NOT provide an alternate path to review documents. Language practice launched from a reading item SHALL return to the Queue/reader source and SHALL not convert the reading item into a review card.
...
#### Scenario: Practice returns to the reading source
- **WHEN** the learner exits practice launched from a Queue reading item
- **THEN** the Queue item and reading position SHALL remain available at the original source location

### Requirement: Document review code paths are removed from the review session

The review session's data model and UI SHALL NOT carry a document variant. Specifically: the `ReviewDocumentItem` type, the `ReviewDocumentCard` component, the document interleaving in `loadQueue`, the document branch in `submitRating` (the `rateDocument` call), and the auto-reveal-answer behavior keyed on a document being current SHALL be removed. Every card in a session SHALL be a flashcard routed through the flashcard scheduler. Language practice SHALL use its own attempt/evidence path and SHALL not call review rating APIs.
...
#### Scenario: Practice cannot rate a review card implicitly
- **WHEN** a learner submits a practice answer for a source sentence associated with a learning item
- **THEN** the practice result SHALL not invoke `submitReview`, change a rating, or reschedule the item without an explicit learner action
