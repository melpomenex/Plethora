...
## 2. Native plugins
- [x] 2.1 `src-tauri/plugins/plethora-storekit` (StoreKit 2): products, purchase, current entitlements, transaction updates, restore, app_account_token binding
- [x] 2.2 `src-tauri/plugins/plethora-playbilling` (Play Billing 7+): equivalent surface + obfuscated_account_id
...
- [x] 3.2 `POST /v1/billing/validate` — provider adapters verifying against App Store Server API v2 (JWS), Play Developer API, web provider API
- [x] 3.3 `GET /v1/billing/subscriptions` (unified view) + `POST /v1/billing/restore`
- [x] 3.4 Webhooks `/v1/billing/webhooks/{provider}`: signature verification, event-id dedupe, out-of-order guards
- [x] 3.5 Grant-derivation config (product→capabilities/quotas; refund/grace policy knobs) + unit matrix tests
...
- [x] 4.1 Purchase intent path: CapabilityGate reason → paywall hook → provider purchase → validate → entitlement refresh event
...
## 5. Validation
- [x] 5.1 Adapter tests with recorded store fixtures (incl. revoked certs, refund/grace/expiry matrices)
- [x] 5.2 Webhook idempotency + signature-rejection tests
- [x] 5.3 Client flow tests with mock provider (offline transaction delivery, abandoned purchase cleanup)
- [x] 5.4 Sandbox end-to-end verification checklist per platform (documented for proposal 23)
