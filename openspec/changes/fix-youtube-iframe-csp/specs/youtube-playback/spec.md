...
#### Scenario: YouTube IFrame API loads without CSP violation
- **GIVEN** the application CSP is configured in `tauri.conf.json`
- **WHEN** the YouTube IFrame Player API script is loaded from `http://www.youtube.com/iframe_api`
- **THEN** the CSP `script-src` and `script-src-elem` directives SHALL allow `http://www.youtube.com`
- **AND** no CSP violation error SHALL occur
