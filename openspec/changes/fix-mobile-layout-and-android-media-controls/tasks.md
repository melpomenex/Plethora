...
- [x] 7.1 Set up diagnostics: `adb logcat -c` then `adb logcat | grep -Ei 'Plethora|RemoteMediaSessionService|MediaSession|Media3|ForegroundService|AndroidRuntime'`; confirm the new lifecycle/metadata/promotion logs appear and are non-spammy.
  - Result (2026-08-21, Pixel 9 Pro XL / Android 16, release build): initial run produced ZERO PlethoraMedia lines — app.tauri.Logger is BuildConfig.DEBUG-gated and silent in release. Fixed by switching media diagnostics to direct android.util.Log (tag "PlethoraMedia"). Second run: logs visible, non-spammy (player-state transitions deduped), and immediately exposed the real root cause: update_media_metadata deserialization failed in release ("Cannot construct instance of t3.B — no Creators") because UpdateMediaMetadataArgs/AckMediaCommandsArgs/DiscardMediaCommandsArgs lacked @InvokeArg, so release R8 renamed/stripped them (debug builds unaffected). Fixed + consumer-rules keep rule updated from the stale pre-rebrand package. Third run: snapshots apply every second, state=playing/paused tracked.
- [ ] 7.2 During playback run `adb shell dumpsys media_session` and record: exactly one active Plethora session with correct metadata/package while playing; absent after teardown. Also check `adb shell dumpsys notification` for the media notification and POST_NOTIFICATIONS grant state.
  - Interim result (2026-08-21): POST_NOTIFICATIONS granted=true (USER_SET); dumpsys media_session shows exactly one com.plethora.app session, active=true, as the Media button session, PlaybackState PAUSED position=87s tracking the app, seek/play-pause actions advertised, no duplicate sessions. Remaining: teardown absence check + notification dumpsys.
