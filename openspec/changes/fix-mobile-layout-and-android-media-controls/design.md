...
- **Pre-existing, out-of-scope failures observed at verification time:** `precisionScheduler.test.ts` native-fixture mismatch (caused by the branding commit's Rust `precision -> precision` rename) and one flaky `ReviewCard` occlusion case that passes in isolation; 9 pre-existing lint errors in untouched files.
