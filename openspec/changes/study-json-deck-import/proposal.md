...
- Transform the Study JSON fields to be compatible with Plethora Classic, Plethora Adaptive, Plethora Precision, and FSRS-6 algorithms
...
- **Rust backend**: New module `src-tauri/src/study_json_import.rs` alongside existing `anki.rs` and `legacy_third_party_import.rs`
...
- **Algorithms**: Imported cards default to Plethora Classic (compatible with ease_factor/interval/repetitions fields in the JSON). Users can switch algorithms post-import. For Plethora Precision, initial `PrecisionState` can be seeded from the JSON fields.
