...
- [x] 10.2 Add the Mother Jones regression fixture with the exact metadata assertions (Mother Jones / title / Sophie Hurwitz / August 15, 2026), hero figure/caption presence, and absence of the reported chrome strings; assert no site-specific rule fires.
- [x] 10.3 Build the fixture-driven suite runner (mocked fetch from fixtures; fake rendered capture for the JS-shell case) and make it pass; verify zero network usage (`src/utils/articleImport/__tests__` offline run).
