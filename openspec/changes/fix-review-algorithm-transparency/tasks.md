...
- [x] 1.2 In `src/components/review/ReviewTransparencyPanel.tsx`, detect Adaptive/Precision state from `card.algorithm_state` content (not from the label algorithm) so reps/lapses/retrievability stats still display correctly for cards with stored Plethora state.
