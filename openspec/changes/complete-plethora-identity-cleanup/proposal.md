...
1. **Source Identifiers and Components**: Files and symbols such as `src/lib/supermemo-grades.ts`, `SuperMemoRatingControl.tsx`, `src-tauri/src/algorithms/supermemo.rs`, `src-tauri/src/algorithms/sm18.rs`, and `src-tauri/src/algorithms/sm20/` expose third-party names to active development.
2. **Product Documentation & RAG Corpus**: Dedicated markdown documents such as `docs/product/features/scheduling/sm18-algorithm.md`, `docs/product/features/scheduling/sm20-postpone.md`, and `docs/product/features/imports/supermemo-zip.md` teach third-party algorithm provenance and are ingested into the "Ask Plethora" help search index (`helpIndex.json`), causing product help queries to return third-party explanations.
3. **Dedicated Migration/Import Subsystems**: `src/utils/supermemoImport.ts` and `src-tauri/src/supermemo_import.rs` implement a proprietary XML zip parser solely for migrating collections from a legacy commercial software package.
4. **Database & Serialization Compatibility Literals**: SQLite records in `learning_items.algorithm_type` store literal strings (`"sm18"`, `"sm20"`, `"sm2"`, etc.), and SQLite tables are named `sm20_arena`, `sm20_m2_optimizer`, etc.
...
This proposal establishes a **permanent, Plethora-native architecture** that cleanly eliminates all third-party SuperMemo branding, terminology, documentation, and implementation-facing naming while **preserving 100% of Plethora's working scheduling mathematics, user review data, and desktop/mobile UX**.

---

## Goals

1. **Zero Unintended References**: Ensure that a repository-wide case-insensitive audit for `supermemo`, `super memo`, `sm18`, `sm20`, `sm15`, `sm19`, and related terms yields zero unintended occurrences across active source, comments, tests, benchmarks, docs, and assets.
...
3. **Safe, Lossless Persistence Migration**: Introduce an automatic SQLite schema and data migration (along with frontend settings store migration) that maps legacy `algorithm_type` identifiers (`"sm18"`, `"sm20"`, `"sm2"`, etc.) to canonical Plethora identifiers (`"adaptive"`, `"precision"`, `"classic"`, etc.) with zero data loss or preference reset for existing users.
4. **Clean Removal of Dedicated SuperMemo Import**: Fully remove the legacy SuperMemo XML/ZIP import feature (backend parser, Tauri commands, frontend utility, file picker option, route handling, and docs) without affecting independent import formats (Anki `.apkg`, Kindle clippings, Markdown directory, JSON deck, RSS, URL).
...
| **2. Product Documentation** | `docs/product/features/scheduling/sm18-algorithm.md`, `sm20-postpone.md`, `sm20-arena.md`, `imports/supermemo-zip.md`, `concepts/incremental-reading.md`, `queue/extract-chains.md` | Delete `sm18-algorithm.md` and `supermemo-zip.md`. Replace with `docs/product/features/scheduling/adaptive-scheduler.md`, `precision-scheduler.md`, `precision-postpone.md`, and clean `extract-chains.md` / `incremental-reading.md`. |
| **3. Internal Source Identifiers** | `src/lib/supermemo-grades.ts`, `src/components/review/SuperMemoRatingControl.tsx`, `src/utils/queueScrollOrder.ts`, `src/pages/queueScrollKeyboard.ts` | Rename to `src/lib/rating-grades.ts`, `SixGradeRatingControl.tsx`, `usesSixGradeKeys`, `RatingSchema.type = "six-grade" \| "four-grade"`, `SIX_GRADE_SCALE`. |
| **4. Scheduler & Algorithm Engines** | `src-tauri/src/algorithms/supermemo.rs`, `sm18.rs`, `sm18_data.rs`, `sm20/`, `src/lib/sm18.ts`, `sm20.ts`, `sm20Collection.ts` | Rename to `src-tauri/src/algorithms/classic.rs`, `adaptive.rs`, `adaptive_data.rs`, `precision/` (`arena.rs`, `kernel.rs`, etc.), `src/lib/adaptiveScheduler.ts`, `precisionScheduler.ts`, `precisionCollection.ts`. |
| **5. Persistent Database & Settings** | SQLite `learning_items.algorithm_type`, `sm20_arena`, `sm20_m2_optimizer`, `sm20_m3_matrices`, `sm20_model_params`, `settingsStore.learning.algorithm`, `sm20PureM4`, `sm20ArenaReviewMode` | Schema migration `086_complete_plethora_identity_migration`: update `algorithm_type` values (`sm18` $\to$ `adaptive`, `sm20` $\to$ `precision`, `sm2` $\to$ `classic`), rename tables (`arena_state`, `arena_m2_optimizer`, `arena_m3_matrices`, `arena_model_params`), migrate settings keys to `precisionPureKernel`, `arenaReviewMode`. |
| **6. Import/Export Subsystem** | `src-tauri/src/supermemo_import.rs`, `src/utils/supermemoImport.ts`, `EnhancedFilePicker.tsx`, `src/routes/documents.tsx`, `src-tauri/src/lib.rs` | **Complete removal**: Delete Rust parser and TS utility; remove `import_supermemo_package` and `validate_supermemo_package` commands; remove `"supermemo"` from `ImportSource` and file picker filters; drop `supermemo://` protocol references. |
| **7. Tests, Fixtures & Benchmarks** | `sm18.test.ts`, `sm20.test.ts`, `supermemo-grades.test.ts`, `SuperMemoRatingControl.matrix.test.tsx`, `sm20ArenaParityFixture.json`, `sm20.bench.ts`, `scripts/perf-baselines.json` | Rename tests to `adaptiveScheduler.test.ts`, `precisionScheduler.test.ts`, `ratingGrades.test.ts`, `SixGradeRatingControl.matrix.test.tsx`, fixture `arenaParityFixture.json`, benchmark `precision.bench.ts` (`precision-scheduler/review-sequence`), update `perf-baselines.json`. |
...
| **9. Current OpenSpec Specs** | `openspec/specs/postpone-engine/`, `specs/document-rating/`, `specs/scheduler-catalog/` | Sanitize any residual SM-18/SM-20/SuperMemo strings in active specification requirements and scenarios. |
| **10. OpenSpec Change History** | `openspec/changes/fix-supermemo-zip-import/`, `supermemo-faithful-queue/`, `unify-supermemo-rating-ux/`, `add-sm18-*`, `fix-sm20-*` | Sanitize or archive completed change directories under neutral naming (`legacy-rating-ux`, `unified-priority-queue`, etc.) so active search tools find no stale branding. |
...
| **12. External URLs & Citations** | `super-memory.com`, `supermemo.guru`, decompilation links in code headers | Remove all third-party website links and decompilation notes; replace with direct, clean mathematical documentation. |
...
| :--- | :--- | :--- | :--- |
| `sm18` / `SM18` / `SM-18` | `adaptive` | **Plethora Adaptive** | Spaced repetition scheduler using a continuous 3-dimensional stability increase interpolation matrix across Difficulty, Stability, and Retrievability. |
| `sm20` / `SM20` / `SM-20` | `precision` | **Plethora Precision** | Spaced repetition scheduler powered by an Algorithm Arena multi-model ensemble (kernel model, Bayesian matrix updates, and adaptive per-user model weighting). |
| `sm2` / `SM2` / `SM-2` | `classic` | **Plethora Classic** | Classical factor-based interval growth scheduler with ease factor adjustment. |
| `sm5` / `sm8` / `sm15` | `classic_5` / `classic_8` / `classic_15` | **Plethora Classic (Legacy N)** | Historical baseline interval algorithms used for research and arena comparison. |
| `fsrs` | `fsrs` | **FSRS-6** | Modern third-party power-curve spaced repetition scheduler (preserved as-is). |
| `supermemo-grades.ts` | `rating-grades.ts` | — | Single source of truth for the 6-point (0–5) and 4-point (1–4) grading scales. |
| `SuperMemoRatingControl` | `SixGradeRatingControl` | — | UI component rendering touch joystick and 6-button grid for 6-grade schedulers. |
| `SuperMemoGrade` | `SixGradeDefinition` | — | Definition for grades 0 through 5. |
| `SUPERMEMO_GRADES` | `SIX_GRADE_SCALE` | — | Constant array of six rating grade definitions. |
| `SUPERMEMO_RATING_SCHEMA` | `SIX_GRADE_RATING_SCHEMA` | — | Rating schema identifier (`type: "six-grade"`). |
| `sm20PureM4` | `precisionPureKernel` | — | Settings toggle for using the direct 35-parameter theoretical kernel without ensemble blending. |
| `sm20ArenaReviewMode` | `arenaReviewMode` | — | Settings toggle for automatic vs. manual candidate interval selection. |
...
- **Removed Code**: `src-tauri/src/supermemo_import.rs`, `src/utils/supermemoImport.ts`, `docs/product/features/imports/supermemo-zip.md`, legacy import commands and UI handlers.
