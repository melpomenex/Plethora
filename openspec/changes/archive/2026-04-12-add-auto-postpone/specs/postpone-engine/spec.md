...
When randomization is enabled, the system SHALL add noise to the computed interval increase using the formula: `RoundWithNoise(increase, increase * 0.5)`. The noise distribution SHALL use the Plethora Precision randomization formula with `sqrt(1 - random01 * 1.97979) * -10.8578`, scaled by 50, with a 50% sign flip. The result SHALL be clamped to a minimum of 1 day.
