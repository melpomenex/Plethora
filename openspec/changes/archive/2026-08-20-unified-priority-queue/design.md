# Design — Plethora Faithful Queue

## Two queues, not one (the key correction)

Plethora has **two distinct queues** with different purposes, backing objects,
...
Source: [official Priority Queue docs](https://help.legacy-third-party.org/wiki/Priority_queue),
[glossary](https://third-party SRS site/help/g.htm), and the decompiled binary's
two separate queue objects. Full documentation in
`~sushi/precision-kernel-re/priority_queue_algorithm.md` (stub) and
...
1. **Neural-queue activation is constant (0.05).** We never need to derive
   urgency from learning state — Plethora doesn't. Urgency is *positional* in
   the priority queue.
2. **Priority is derived from queue position** via linear interpolation. There
   ...
   reposition in.
3. **The tree maps to SQLite cleanly.** Plethora's `contents.dat` /
...
Plethora has one `elements` table with a type byte. Incrementum has three
...
urgency*, not the user's importance ranking. Plethora's priority queue treats
...
Mirrors Plethora's `FUN_00cb1630` and `FUN_00cb21c0`:
...
Plethora does NOT use a pure priority sort. From the [docs](https://help.legacy-third-party.org/wiki/Priority_queue)
...
When outstanding material exceeds what the student can get through, Plethora
auto-postpones low-priority excess. The algorithm is fully reverse-engineered
in `~sushi/precision-kernel-re/postpone_algorithm.md` (manual + auto-postpone, per-branch
...
Mirrors Plethora's *Learn : Go neural*: an **opt-in mode**, not the normal
...
This is layered on top of priority-queue learning, exactly as in Plethora:
...
- **No new scheduling algorithm.** FSRS/Plethora Adaptive/Plethora Precision stability/difficulty/
