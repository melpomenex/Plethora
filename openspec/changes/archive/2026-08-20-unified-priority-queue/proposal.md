...
Today. The two types never interleave. The user wants to emulate Plethora-style
...
2. **The existing priority scorer is a per-item weighted sum**
   ...
   but it is architecturally unlike Plethora. Plethora incremental reading is
...
Plethora has **two distinct queues**, which the first draft of this proposal
conflated. Mining the decompiled binary (`~sushi/precision-kernel-re`) plus the
[official docs](https://help.legacy-third-party.org/wiki/Priority_queue) and
[glossary](https://third-party SRS site/help/g.htm) made the split unambiguous:
...
(commit `8f401c20` on `~sushi/precision-kernel-re`). **The neural queue reads from the
...
- **Auto-sort the learning session** by Plethora's combined criterion
...
- **Activation fires when the neural queue runs low**, seeded at the current
  element (mirrors Plethora's phase-3 "serve next / refill").
- This is an **optional, opt-in mode** (Plethora's *Learn : Go neural*),
...
- FSRS/Plethora Adaptive/Plethora Precision **scheduling** (stability/difficulty/due-date) is untouched.
...
- `priority-queue` (Phase 3): every element (including cards) has user-set
  priority; the learning session is auto-sorted by Plethora's combined
  criterion with auto-postpone of overflow.
- `neural-queue` (Phase 4): Plethora's spreading-activation neural queue for
