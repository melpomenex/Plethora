...
### Requirement: Extended PrecisionState for FSRS branch
`PrecisionState` SHALL include an `algorithm_branch` field (defaulting to 0 for classic) and FSRS-family fields: `retrov`, `s_factor`, `multiplier`, with defaults that preserve backward compatibility.

#### Scenario: Parsing legacy state without new fields
- **WHEN** an existing `PrecisionState` JSON is deserialized that lacks `algorithm_branch`, `retrov`, `s_factor`, `multiplier`
