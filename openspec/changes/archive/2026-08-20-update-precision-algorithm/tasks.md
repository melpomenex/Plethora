...
- [x] 1.1 Add `FSRS_PARAMS: [f64; 35]` constant array to `precision.rs`
...
- [x] 2.5 Implement `fsrs_init_item(grade, stability, flag)` returning `PrecisionState`

## 3. Rust — State Extension & Dispatch

- [x] 3.1 Add `algorithm_branch`, `retrov`, `s_factor`, `multiplier` fields to `PrecisionState` with backward-compatible defaults
...
- [x] 4.5 Add backward-compatibility test: parse old PrecisionState JSON without new fields

## 5. TypeScript — FSRS Constants & Expert Functions

- [x] 5.1 Add `FSRS_PARAMS: readonly number[]` constant array to `precisionScheduler.ts`
...
- [x] 6.5 Implement `fsrsInitItem(grade, stability, flag)` returning `PrecisionState`

## 7. TypeScript — State Extension & Dispatch

- [x] 7.1 Add `algorithm_branch`, `retrov`, `s_factor`, `multiplier` fields to `PrecisionState` interface with defaults
- [x] 7.2 Update `parsePrecisionState()` to default new fields for backward compatibility
- [x] 7.3 Update `precisionReview()` to dispatch based on `algorithm_branch`
- [x] 7.4 Update `precisionPreviewIntervals()` to handle both branches
