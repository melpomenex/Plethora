# flashcard-studio-mobile-controls Specification

## Purpose

Defines the consolidated mobile presentation of the AI Flashcard Studio's configuration controls. On a phone the Studio previously stacked four full-width control bands — header, context bar, Context Control accordion, and panel toggle — above the conversation, leaving the message list under 20% of the viewport. Those controls are configuration rather than conversation: set once at the start of a session and rarely touched again. This capability collapses them into a single-row context chip bar that summarizes current state, opens each control's full UI in a bottom sheet, moves secondary view modes into an overflow menu, and guarantees the conversation a minimum share of the viewport. It is a presentation-layer concern only — every control keeps its existing handler, state, and semantics, and the desktop layout is unchanged.
