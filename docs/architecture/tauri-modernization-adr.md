...
| App `permissions/app-command-acl.toml` via `build.rs` | Tauri 2.11.1+ ACL-enforces localhost IPC; `remote.urls` alone is insufficient without explicit app command grants |
